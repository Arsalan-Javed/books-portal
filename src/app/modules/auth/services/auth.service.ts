...
export class AuthService implements OnDestroy {
  ...
  login(email: string, password: string): Observable<any> {
    ...
    return from(this.authServiceFire.login(email, password)).pipe(
      map((auth: any) => {
        const result = this.setAuthFromLocalStorage(auth);
        return result;
      }),
      switchMap((user: any) => {
        const currentUser = this.authServiceFire.getCurrentUser();
  ...
  //   this.isLoadingSubject.next(true);
  //   return from(this.authServiceFire.getCurrentUser()).pipe(
  //     map((user: any) => {
  //       if (user) {
  //         this.currentUserSubject.next(user);
  //       } else {
